...
class WAConnection extends _3_Connect_1.WAConnection {
    ...
    chatAddMessage(message, chat) {
        ...
        if (protocolMessage) {
            ...
            const messageTimestamp = Utils_1.toNumber(message.messageTimestamp);
            let idx = messages.length - 1;
            for (idx; idx >= 0; idx--) {
                if (Utils_1.toNumber(messages[idx].messageTimestamp) <= messageTimestamp) {
                    break;
                }
            }
            // if the message is already there
            if (((_b = messages[idx]) === null || _b === void 0 ? void 0 : _b.key.id) === message.key.id)
                return;
